...
echo "</form>";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($_POST['title'] as $job_id => $title) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssdsii", $title, $location, $salary, $description, $job_id, $hr_account_id);
            if (!$stmt->execute()) {
                // Handle errors here
                die("Error executing statement: " . $stmt->error);
            }
    ...
    echo '<div style="color: red; font-weight: bold;">No jobs found for this HR to edit.</div>';
    // echo "No jobs found for this HR to edit.";
